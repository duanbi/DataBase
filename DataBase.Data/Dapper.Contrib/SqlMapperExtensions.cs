...
namespace DataBase.Data
{
    public static partial class SqlMapperExtensions
    {
        ...
        /// <returns></returns>
        private static List<PropertyInfo> IdentityKeyPropertiesCache(Type type)
        {
            var allProperties = TypePropertiesCache(type);
            var keyProperties = allProperties.Where(p =>
            {
                var property = p.GetCustomAttributes(typeof(KeyAttribute), false).AsList();
                return property.Any() && ((KeyAttribute)property[0]).IsIdentity;
            }).ToList();
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
            {
                ...
                var keyProperty = new PropertyMetadata(key);
                ...
                sql = $"SELECT {columns} FROM {tableName} WHERE {tableName}.{keyProperty.ColumnName} = @id";
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var allProperties = TypePropertiesCache(type);
            var keyProperties = IdentityKeyPropertiesCache(type);
            ...
            if (returnVal <= 0)
            {
                var allKeyProperties = KeyPropertiesCache(type);
                var key = allKeyProperties.FirstOrDefault();
                if (key != null)
                {
                    var keValue = key.GetValue(entityToInsert);
                    int.TryParse(keValue.ToString(), out returnVal);
                }
            }
            return returnVal;
        }
